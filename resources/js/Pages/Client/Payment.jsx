...
const Payment = ({ order, deliverableFiles, paymentSettings, user }) => {
  ...
  const [paymentStatus, setPaymentStatus] = useState(order?.status || 'pending_payment');
  ...
  const handlePaymentCallback = (status, result) => {
    ...
    if (status === 'success') {
      ...
      showToast('Payment successful! Your order has been processed.', 'success');
      checkPaymentStatus();
    } else if (status === 'pending') {
      setPaymentStatus('pending');
      setPollingActive(true);
      showToast('Your payment is being processed. We will notify you when it\'s complete.', 'info');
      checkPaymentStatus();
    } else {
    ...
    try {
      ...
      if (status && status.success) {
        ...
        if (status.status === 'completed') {
          showToast('Payment has been completed! You can now download your files.', 'success');
            ...
            <div className="flex">
              ...
              <div className="ml-3">
                <p className="text-sm text-yellow-700">
                  Your payment is being processed. We will notify you when it's completed.
                </p>
                <button
                  onClick={() => {
                    setIsCheckingManually(true);
                ...
                >
                  {checkingPayment && isCheckingManually ? 'Checking...' : 'Check Payment Status'}
                </button>
              </div>
            </div>
          </div>
            ...
            <div className="flex">
              ...
              <div className="ml-3">
                <p className="text-sm text-green-700">
                  Payment has been successfully completed! You will be redirected to your order details shortly.
                </p>
              </div>
            </div>
          </div>
        ...
        {(paymentStatus === 'completed' || canDownload) && deliverableFiles && deliverableFiles.length > 0 && (
          <div className="mt-6 border rounded-lg overflow-hidden">
            <div className="bg-gray-50 px-4 py-3 border-b">
              <h2 className="text-base font-semibold text-gray-900">Available Files for Download</h2>
            </div>
              ...
              {deliverableFiles.map(file => (
                <div key={file.id} className="px-4 py-3 flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-sm text-gray-500">{(file.file_size / 1024 / 1024).toFixed(2)} MB</p>
                  </div>
                  <a 
            ...
            <div className="flex">
              ...
              <div className="ml-3">
                <p className="text-sm text-red-700">
                  Payment failed. Please try again or contact our support team for assistance.
                </p>
                <button
                  onClick={handlePayNow}
