                            ...
                            <div className="mb-4">
                                ...
                                <div className="mt-2 space-x-2 flex flex-wrap gap-2">
                                    ...
                                    {categories && categories.map(category => (
                                        <Link 
                                            key={category.id} 
                                            href={route('client.services', { category: category.id })} // Assuming you'll handle category filtering in controller
                            ...
                            {services && services.data && services.data.length > 0 ? (
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                    {services.data.map(service => (
                                        <div key={service.id} className="bg-white shadow rounded-lg overflow-hidden flex flex-col">
                                            <Link href={route('client.services.show', service.id)} className="block">
                                                <img 
                                                    src={service.image_url || `https://ui-avatars.com/api/?name=${encodeURIComponent(service.title.substring(0,15))}&background=random&color=fff`} 
                                                    alt={service.title} 
                                            ...
                                            <div className="p-4 flex flex-col flex-grow">
                                                <Link href={route('client.services.show', service.id)} className="block">
                                                    <h4 className="text-lg font-semibold text-gray-900 truncate hover:text-indigo-600" title={service.title}>
                                                        {service.title}
                                                    </h4>
                                                </Link>
                                                <p className="text-sm text-gray-600 mt-1">
                                                    By <Link href={route('client.freelancers.show', service.user.id)} className="text-indigo-600 hover:underline">{service.user.name}</Link>
                                                </p>
                                                ...
                                                <div className="mt-auto pt-3 flex items-center justify-between">
                                                    ...
                                                    <Link 
                                                        href={route('client.services.order.form', service.id)}
                                ...
                                <div className="text-center py-10">
                                    ...
                                    <p className="mt-1 text-sm text-gray-500">There are currently no services available. Please check back later.</p>
                                </div>
                            )}
