...
const Projects = () => {
  ...
  const projects = [
    ...
    },
    {
      ...
      status: 'completed',
      image:
        'https://ui-avatars.com/api/?name=Rini+Wulandari&background=10b981&color=fff',
    },
    {
      id: 5,
      ...
      status: 'cancelled',
      image:
        'https://ui-avatars.com/api/?name=Arya+Wijaya&background=ef4444&color=fff',
    },
  ];
  ...
  const filteredProjects = projects.filter(project => {
    ...
    const matchesStatus =
      filterStatus === 'all' || filterStatus === project.status;
  ...
  const activeProjects = filteredProjects.filter(project =>
    ['ongoing', 'pending'].includes(project.status)
  );
  ...
  return (
    <ClientLayout title='Proyek Saya' subtitle='Kelola semua proyek Anda'>
      {/* Actions Bar */}
      ...
      {filteredProjects.length === 0 && (
        <div className='bg-white rounded-xl shadow-sm p-6 text-center'>
          ...
          <p className='mt-1 text-sm text-gray-500'>
            {searchQuery
              ? 'Coba kata kunci pencarian lain atau reset filter'
