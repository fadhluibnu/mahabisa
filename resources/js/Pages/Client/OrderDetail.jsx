          ...
          <div className="p-6">
            ...
            {order.status === 'delivered' && (
              <div className="mb-6">
                <div className="bg-yellow-50 border-l-4 border-yellow-400 p-4 mb-4">
                  ...
                  <p>The freelancer has completed your order and uploaded the deliverables. Please make payment to access the files.</p>
                </div>
            ...
            {order.status === 'completed' && (
              <div className="mb-6">
                <div className="bg-green-50 border-l-4 border-green-400 p-4 mb-4">
                  ...
                  <p>Your payment has been processed successfully. You can now download all deliverables.</p>
                </div>
            ...
            {order.status === 'pending' && (
              <div className="bg-blue-50 border-l-4 border-blue-400 p-4 mb-4">
                <p>Waiting for the freelancer to accept your order. You'll be notified once they accept.</p>
              </div>
            )}
            
            {order.status === 'in_progress' && (
              <div className="bg-blue-50 border-l-4 border-blue-400 p-4 mb-4">
                <p>The freelancer is working on your order. The expected delivery date is {formatDate(order.expected_delivery_date)}.</p>
              </div>
            )}
          </div>
        </div>
          ...
          <div className="p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
              ...
              <div>
                ...
                <p className="font-medium">{order.freelancer?.name || 'Unknown Freelancer'}</p>
              </div>
              <div>
              ...
              <div>
                ...
                <p className="font-medium">{formatRupiah(order.platform_fee)}</p>
              </div>
              <div>
                <p className="text-gray-600">Total:</p>
                <p className="font-medium text-primary font-bold">{formatRupiah(order.total_amount)}</p>
              </div>
            </div>
              ...
              <div className="overflow-x-auto">
                <table className="min-w-full bg-white">
                  ...
                  <tbody className="divide-y divide-gray-200">
                    {files.map(file => (
                      <tr key={file.id}>
                        <td className="py-3 px-4 whitespace-nowrap">{file.original_name}</td>
                        <td className="py-3 px-4 whitespace-nowrap">{file.file_type}</td>
                        <td className="py-3 px-4 whitespace-nowrap">{(file.file_size / 1024 / 1024).toFixed(2)} MB</td>
                        <td className="py-3 px-4 whitespace-nowrap">
                          {file.status === 'deliverable' && order.status !== 'completed' ? (
                        ...
                        <td className="py-3 px-4 whitespace-nowrap">
                          {file.can_download ? (
                            ...
                            <span className="text-gray-400">
                              {order.status === 'delivered' ? (
                                <Link href={`/client/orders/${order.id}/simple-invoice`} className="text-blue-600 hover:underline">
