...
const Orders = ({ orders, filters, orderCounts }) => {
  // Menerima orders, filters, orderCounts
  ...
  const handleCancelOrder = async () => {
    if (!cancellationReason || cancellationReason.length < 10) {
      showToast(
        'Harap berikan alasan pembatalan yang jelas (minimal 10 karakter)',
        'error'
      );
      return;
    }
      ...
      <div className='container mx-auto py-8 px-4'>
        ...
        {orders.data && orders.data.length > 0 ? (
          ...
          <div className='bg-white shadow-md rounded-lg p-8 text-center'>
            ...
            />{' '}
            {/* Gunakan Icon component */}
            <h3 className='text-xl font-medium text-gray-900 mb-2'>
              Tidak Ada Pesanan
            </h3>
            <p className='text-gray-500 max-w-md mx-auto'>
              {currentTab === 'all'
                ? 'Anda belum memiliki pesanan yang aktif.'
                : currentTab === 'pending'
                  ? 'Tidak ada pesanan yang menunggu konfirmasi saat ini.'
                  : currentTab === 'in_progress'
                    ? 'Tidak ada pesanan yang sedang dikerjakan saat ini.'
                    : currentTab === 'delivered'
                      ? 'Tidak ada pesanan yang sudah terkirim.'
                      : currentTab === 'completed'
                        ? 'Tidak ada pesanan yang telah selesai.'
                        : currentTab === 'pending_payment'
                          ? 'Tidak ada pesanan yang menunggu pembayaran.'
                          : currentTab === 'revision'
                            ? 'Tidak ada pesanan yang perlu direvisi.'
