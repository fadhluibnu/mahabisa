...
const UserEdit = ({ user }) => {
  // In a real app, user data would be passed as a prop from the controller
  ...
  const dummyUser = {
    ...
    bio: 'Web developer with 5 years of experience in React and Laravel.',
    address: 'Jl. Sudirman No. 123, Jakarta Pusat',
    skills: ['Web Development', 'UI/UX Design', 'Mobile Development'],
    active: true,
  };

  const userToEdit = user || dummyUser;
  ...
  const { data, setData, put, processing, errors } = useForm({
    ...
    password: '', // empty for edit, only filled if changing password
  ...
  const [previewImage, setPreviewImage] = useState(userToEdit.profileImage);
  ...
  return (
    <AdminLayout
      title='Edit Pengguna'
      subtitle={`Edit informasi pengguna: ${userToEdit.name}`}
    >
      <Head title={`Edit ${userToEdit.name} - MahaBisa Admin`} />
