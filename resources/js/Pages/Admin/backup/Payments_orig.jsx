...
const Payments = ({ payments, user }) => {
  ...
  const generateReport = () => {
    ...
    .catch(error => {
      ...
      const filteredData = payments.data?.filter(payment => {
      ...
      const totalAmount = filteredData.reduce((sum, payment) => sum + parseFloat(payment.amount || 0), 0);
      const totalCommission = filteredData.reduce((sum, payment) => sum + (parseFloat(payment.amount || 0) * 0.2), 0);
      ...
      {totalPages > 1 && (
        <div className="flex justify-between items-center">
          <div className="text-sm text-gray-500">
            Showing {indexOfFirstTransaction + 1}-{Math.min(indexOfLastTransaction, totalTransactions)} of {totalTransactions} transactions
          </div>
