...
const EarningsDetail = () => {
  ...
  const [earning, setEarning] = useState({
    ...
    project: {
      ...
      description: 'Mengubah tampilan dan meningkatkan performa website e-commerce untuk meningkatkan konversi penjualan.',
      status: 'completed',
      ...
      <div className="bg-white shadow-sm rounded-xl overflow-hidden mb-6">
        ...
        {activeTab === 'overview' && (
          <div className="p-6">
              ...
              <div className="bg-gray-50 p-4 rounded-lg">
                <dl className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-5">
                  ...
                  <div>
                    ...
                    <dd className="mt-1 text-sm text-gray-900">{formatDate(earning.paymentDate)}</dd>
                  </div>
                  <div>
                    <dt className="text-sm font-medium text-gray-500">Metode Pembayaran</dt>
                    <dd className="mt-1 text-sm text-gray-900">
                      {earning.paymentMethod === 'bank_transfer' ? 'Transfer Bank' : earning.paymentMethod}
                    </dd>
                  </div>
                  <div>
                    <dt className="text-sm font-medium text-gray-500">ID Transaksi</dt>
                    <dd className="mt-1 text-sm text-gray-900">{earning.transactionId}</dd>
                  </div>
                </dl>
              </div>
            </div>
              ...
              <div className="bg-gray-50 p-4 rounded-lg">
                <dl className="space-y-3">
                  <div className="flex justify-between">
                    ...
                    <dd className="text-sm font-medium text-gray-900">{formatRupiah(earning.amount)}</dd>
                  </div>
                  <div className="flex justify-between">
                    <dt className="text-sm font-medium text-gray-500">Pajak Penghasilan (10%)</dt>
                    <dd className="text-sm font-medium text-gray-900">- {formatRupiah(earning.tax)}</dd>
                  </div>
                  <div className="flex justify-between">
                    <dt className="text-sm font-medium text-gray-500">Biaya Layanan (5%)</dt>
                    <dd className="text-sm font-medium text-gray-900">- {formatRupiah(earning.serviceFee)}</dd>
                  </div>
                  <div className="border-t border-gray-200 pt-3 flex justify-between">
                    <dt className="text-base font-bold text-gray-900">Total Diterima</dt>
                    <dd className="text-base font-bold text-green-600">{formatRupiah(earning.netAmount)}</dd>
                  </div>
                </dl>
              </div>
            </div>
            ...
            <div>
              <h3 className="text-lg font-medium text-gray-900 mb-4">Detail Proyek</h3>
              <div className="bg-gray-50 p-4 rounded-lg">
                <dl className="grid grid-cols-1 gap-y-3">
                  <div>
                    <dt className="text-sm font-medium text-gray-500">Judul Proyek</dt>
                    <dd className="mt-1 text-sm text-gray-900">{earning.project.title}</dd>
                  </div>
                  <div>
                    <dt className="text-sm font-medium text-gray-500">Deskripsi</dt>
                    <dd className="mt-1 text-sm text-gray-900">{earning.project.description}</dd>
                  </div>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-3">
                    <div>
                      <dt className="text-sm font-medium text-gray-500">Tanggal Mulai</dt>
                      <dd className="mt-1 text-sm text-gray-900">{formatDate(earning.project.startDate)}</dd>
                    </div>
                    <div>
                      <dt className="text-sm font-medium text-gray-500">Tanggal Selesai</dt>
                      <dd className="mt-1 text-sm text-gray-900">{formatDate(earning.project.endDate)}</dd>
                    </div>
                  </div>
                </dl>
              </div>
        ...
        {activeTab === 'transactions' && (
          <div className="p-6">
            <h3 className="text-lg font-medium text-gray-900 mb-4">Riwayat Transaksi</h3>
          ...
          <div className="p-6">
            ...
            <div className="bg-gray-50 p-4 rounded-lg">
              <div className="flex items-center mb-6">
                ...
                />
                <div className="ml-4">
                  <h4 className="text-lg font-medium text-gray-900">{earning.client.name}</h4>
                  <Link
              ...
              <dl className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-3">
                <div>
                  ...
                  <dd className="mt-1 text-sm text-gray-900">{earning.client.contact}</dd>
                </div>
                <div>
                  <dt className="text-sm font-medium text-gray-500">Telepon</dt>
                  <dd className="mt-1 text-sm text-gray-900">{earning.client.phone}</dd>
                </div>
              </dl>
            </div>
          </div>
