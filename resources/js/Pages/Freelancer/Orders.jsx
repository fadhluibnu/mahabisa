...
const Orders = ({ orders, filters, orderCounts }) => {
  ...
  const handleCancelOrder = async () => {
    if (!cancellationReason || cancellationReason.length < 10) {
      showToast('Please provide a detailed cancellation reason (minimum 10 characters)', 'error');
      return;
    }
    ...
    try {
      await axios.post(`/orders/${selectedOrder.id}/cancel`, {
    ...
    <FreelancerLayout>
      ...
      >
        <div className="p-6">
          ...
          <p className="mb-4 text-red-600">Peringatan: Tindakan ini tidak dapat dibatalkan dan mungkin mempengaruhi penilaian Anda.</p>
          ...
          <div className="flex">
            <div className="relative flex-grow">
              ...
              <input
                ...
                className="pl-10 pr-4 py-2 w-full border border-gray-300 rounded-l-lg focus:ring-primary focus:border-primary"
              />
            </div>
            <button 
              type="submit"
        ...
        {orders.data && orders.data.length > 0 ? (
            ...
            <table className="w-full table-auto">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Jasa</th>
              ...
              <tbody className="divide-y divide-gray-200">
                {orders.data.map((order) => (
                  <tr key={order.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{order.client?.name || '-'}</td>
          ...
          <div className="bg-white shadow-md rounded-lg p-8 text-center">
            ...
            <p className="text-gray-500 max-w-md mx-auto">
              {currentTab === 'all' ? 'Anda belum memiliki pesanan. Pastikan layanan Anda sudah aktif dan terlihat oleh client.' : 
               currentTab === 'pending' ? 'Anda tidak memiliki pesanan yang menunggu konfirmasi saat ini.' : 
               currentTab === 'in_progress' ? 'Anda tidak memiliki pesanan yang sedang dikerjakan saat ini.' :
               currentTab === 'delivered' ? 'Anda tidak memiliki pesanan yang sudah terkirim dan menunggu persetujuan.' :
               currentTab === 'completed' ? 'Anda belum memiliki pesanan yang telah selesai.' :
