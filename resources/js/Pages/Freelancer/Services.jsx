...
const Services = ({
  ...
  const toggleServiceStatus = serviceId => {
    ...
    axios
      .put(`/freelancer/services/${serviceId}/toggle-status`)
      .then(response => {
  ...
  const deleteService = async () => {
    ...
    if (!serviceToDelete || deleteInProgress) {
      console.log(
        'Cannot delete - either no service selected or delete already in progress'
      );
      return;
    }
    ...
    try {
      console.log(
        `Sending delete request for service ID ${serviceToDelete.id}`
      );
      const response = await axios.delete(
        `/freelancer/services/${serviceToDelete.id}`
      );
      ...
      if (error.response?.data?.error) {
        ...
        errorMessage =
          'Tidak dapat menghapus layanan. Pastikan Anda terhubung ke internet.';
      }
      ...
      <div className='mb-6 flex flex-col md:flex-row md:items-center md:justify-between gap-4'>
        <div className='flex-grow'>
          <h2 className='text-lg font-medium text-gray-900'>
            Daftar Layanan ({services.filter(s => s.status === 'active').length}{' '}
            Aktif)
          </h2>
          <p className='text-sm text-gray-500 mt-1'>
            {services.reduce(
              (total, service) => total + (service.completedOrders || 0),
              0
            )}{' '}
            pesanan selesai &bull;{' '}
            {services.reduce(
              (total, service) => total + (service.activeOrders || 0),
              0
            )}{' '}
            pesanan aktif
          </p>
                    ...
                    {service.ordersCount > 0 ? (
                      ...
                      <button
                        onClick={e => {
                          ...
                          e.stopPropagation();
                          console.log(
                            'Delete button clicked for service:',
                            service.id
                          );
          ...
          {searchTerm || filterCategory !== 'all' || filterStatus !== 'all' ? (
            <>
              ...
              <p className='mt-1 text-sm text-gray-500'>
                Coba ubah filter pencarian Anda untuk menemukan layanan yang
                dicari.
                {searchTerm && (
                ...
                {filterCategory !== 'all' && (
                  <span className='block mt-1'>
                    Kategori:{' '}
                    <span className='font-medium'>{filterCategory}</span>
                  </span>
                )}
                {filterStatus !== 'all' && (
                  <span className='block mt-1'>
                ...
                <div className='mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left'>
                  ...
                  <div className='mt-2'>
                    <p className='text-sm text-gray-500'>
                      ...
                      {serviceToDelete.title}"? Tindakan ini tidak dapat
                      dibatalkan.
                    </p>
                  </div>
                </div>
