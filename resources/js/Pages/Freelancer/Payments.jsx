...
const Payments = () => {
  ...
  const handleWithdrawal = () => {
    ...
    const selectedMethod = paymentMethods.find(m => m.id === withdrawalData.paymentMethodId);
    if (!selectedMethod) {
  ...
  const handleAddPaymentMethod = () => {
    if (newPaymentMethod.type === 'bank') {
      if (!newPaymentMethod.bankName || !newPaymentMethod.accountNumber || !newPaymentMethod.accountHolder) {
  ...
  const calculateBalance = () => {
    return transactions.reduce((total, transaction) => {
      ...
      } else if (transaction.type === 'withdrawal' && (transaction.status === 'completed' || transaction.status === 'pending')) {
    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 sm:p-6">
          ...
          <div className="mt-1 flex items-baseline">
            <div className="text-2xl font-semibold text-gray-900">{formatRupiah(calculatePendingBalance())}</div>
          </div>
          <p className="mt-4 text-sm text-gray-500">Saldo ini akan tersedia setelah klien menyelesaikan pembayaran</p>
        </div>

        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 sm:p-6">
          <h3 className="text-sm font-medium text-gray-500">Total Penghasilan Bulan Ini</h3>
          <div className="mt-1 flex items-baseline">
            <div className="text-2xl font-semibold text-gray-900">{formatRupiah(5300000)}</div>
            <span className="ml-2 text-sm text-green-600">+12.5%</span>
          </div>
          <p className="mt-4 text-sm text-gray-500">Dibandingkan dengan bulan lalu</p>
        </div>
      </div>
                  ...
                  <tr key={transaction.id}>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center">
                        {transaction.type === 'project' && transaction.client && (
                          <img
                        ...
                        <div>
                          ...
                          {transaction.type === 'project' && transaction.client && (
                            <div className="text-sm text-gray-500">{transaction.client.name}</div>
                          )}
                          {transaction.type === 'withdrawal' && (
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className={`text-sm font-medium ${transaction.type === 'withdrawal' ? 'text-red-600' : 'text-green-600'}`}>
                        {transaction.type === 'withdrawal' ? '-' : '+'}{formatRupiah(transaction.type === 'project' ? transaction.net : transaction.amount)}
                      </div>
          ...
          <div className="p-4 sm:p-6">
            <div className="space-y-4">
              {paymentMethods.map((method) => (
                ...
                >
                  <div className="flex items-center">
                    <div className={`h-10 w-10 rounded-full flex items-center justify-center ${method.type === 'bank' ? 'bg-blue-100' : 'bg-green-100'}`}>
                      {method.type === 'bank' ? (
                        <svg
            ...
            {paymentMethods.length === 0 && (
              <div className="text-center py-6">
                ...
                <h3 className="mt-2 text-sm font-medium text-gray-900">Tidak ada metode pembayaran</h3>
                <p className="mt-1 text-sm text-gray-500">
