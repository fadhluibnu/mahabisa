...
export default function OrderDetail({ order, files, messages, user, timeRemaining, paymentInfo, canDeliver, hasDeliverableFiles }) {
  ...
  React.useEffect(() => {
    ...
    if (!token) {
      ...
      showToast('Halaman tidak memiliki CSRF token. Coba muat ulang halaman.', 'error');
    } else {
  ...
  const handleFileChange = (e) => {
    ...
    if (tooLargeFiles.length > 0) {
      showToast(`File ${tooLargeFiles[0].name} terlalu besar. Ukuran maksimum adalah 50MB.`, 'error');
      return;
    }
    ...
    const invalidFiles = fileArray.filter(file => {
      ...
      const validExtensions = ['pdf', 'doc', 'docx', 'jpg', 'jpeg', 'png', 'gif', 'zip', 'rar', 'mp3', 'mp4', 'avi', 'mov'];
      return !allowedTypes.includes(file.type) && !validExtensions.includes(ext);
    });
    
    if (invalidFiles.length > 0) {
      showToast(`File ${invalidFiles[0].name} memiliki format yang tidak didukung.`, 'error');
      return;
    }
  ...
  const handleDeliverySubmit = async (e) => {
    ...
    if (deliveryMessage.length < 10) {
      showToast('Harap berikan pesan pengiriman yang lebih detail (minimal 10 karakter)', 'error');
      return;
    }
    ...
    try {
      ...
      const response = await axios.post(`/freelancer/orders/${order.id}/deliver`, formData, {
        headers: {
      ...
      if (response.data.success) {
        ...
        showToast(response.data.message || 'Terjadi kesalahan saat mengirim hasil', 'error');
      }
    } catch (error) {
      ...
      if (error.response) {
        ...
        if (error.response.data && typeof error.response.data === 'string' && error.response.data.includes('SQLSTATE')) {
          showToast('Database error: Terjadi kesalahan pada database. Hubungi administrator.', 'error');
        }
        // More specific error messages based on status codes
          ...
          showToast(firstError ? firstError[0] : 'Validasi gagal, periksa kembali data Anda', 'error');
        } else if (error.response.status === 413) {
          showToast('File yang diunggah terlalu besar. Maksimal 50MB per file.', 'error');
        } else if (error.response.status === 500) {
          showToast(`Error server: ${errorMessage}. Coba lagi atau hubungi administrator.`, 'error');
        } else {
          showToast(errorMessage, 'error');
        }
      } else if (error.request) {
        // No response received (network issue)
        showToast('Tidak ada respon dari server. Periksa koneksi internet Anda dan coba lagi.', 'error');
      } else {
  ...
  const handleRevisionSubmit = async (e) => {
    ...
    if (revisionMessage.length < 10) {
      showToast('Harap berikan detail permintaan revisi yang jelas (minimal 10 karakter)', 'error');
      return;
    }
    ...
    try {
      ...
      showToast(error.response?.data?.message || 'Gagal mengirim permintaan revisi', 'error');
    } finally {
      setIsSubmitting(false);
    }
  };
      ...
      case 'rar':
        return (
          <svg className="w-6 h-6 text-yellow-500" fill="currentColor" viewBox="0 0 20 20">
            <path fillRule="evenodd" d="M4 4a2 2 0 012-2h8a2 2 0 012 2v12a2 2 0 01-2 2H6a2 2 0 01-2-2V4zm2 2v2h2V6H6zm2 2v2h2V8H8zm2 2v2h2v-2h-2zm2 2v2h-2v-2h2z" clipRule="evenodd" />
          </svg>
        );
      default:
        return (
  ...
  return (
    <FreelancerLayout>
      ...
      <Modal show={showRevisionModal} onClose={() => setShowRevisionModal(false)}>
        <div className="p-6">
          <h3 className="text-lg font-bold mb-4">Minta Revisi atau Klarifikasi</h3>
          <p className="mb-4 text-sm text-gray-600">
            Minta informasi tambahan atau klarifikasi dari klien untuk menyelesaikan pesanan ini.
          </p>
      ...
      <div className="py-6 px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2">
              ...
              <div className="px-4 py-5 sm:p-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-gray-900">{order.client?.name || 'N/A'}</p>
                  </div>
                  
                  <div>
                    <h4 className="text-sm font-medium text-gray-500">Layanan</h4>
                    <p className="mt-1 text-sm text-gray-900">{order.service?.title || order.project?.title || 'Layanan Kustom'}</p>
                  </div>
                  
                  <div>
                    <h4 className="text-sm font-medium text-gray-500">Jumlah</h4>
                    <p className="mt-1 text-sm text-gray-900">{formatCurrency(order.total_amount || order.amount || 0)}</p>
                  </div>
                  
                  <div>
                    <h4 className="text-sm font-medium text-gray-500">Tanggal Jatuh Tempo</h4>
                    <p className="mt-1 text-sm text-gray-900">{formatDate(order.due_date) || 'Tidak ditentukan'}</p>
                  </div>
                </div>
                
                {timeRemaining && (
                  <div className="mt-4 p-3 bg-blue-50 rounded-md">
                    <h4 className="text-sm font-medium text-blue-800">Sisa Waktu</h4>
                    {timeRemaining.overdue ? (
                      ...
                      <p className="mt-1 text-sm text-blue-600">
                        {timeRemaining.days} hari, {timeRemaining.hours} jam, {timeRemaining.minutes} menit
                      </p>
                    )}
                  </div>
                )}
            ...
            <div className="bg-white shadow-sm rounded-lg mb-6">
              <div className="px-4 py-5 sm:p-6">
                <h3 className="text-lg font-medium text-gray-900 mb-4">Persyaratan Pesanan</h3>
                <div className="prose prose-sm max-w-none text-gray-800">
                  {order.requirements ? (
                    <p style={{ whiteSpace: 'pre-wrap' }}>{order.requirements}</p>
                  ) : (
                    <p className="text-gray-500 italic">Tidak ada persyaratan khusus yang diberikan.</p>
                  )}
                </div>
              </div>
            </div>
            ...
            <div className="bg-white shadow-sm rounded-lg mb-6">
              <div className="px-4 py-5 sm:p-6">
                ...
                {/* Deliverable Files */}
                <div className="mb-6">
                  <h4 className="text-sm font-medium text-gray-700 mb-3">File Hasil Pekerjaan</h4>
                  {files.filter(file => file.file_type === 'deliverable').length > 0 ? (
                    <ul className="divide-y divide-gray-200">
                      {files.filter(file => file.file_type === 'deliverable').map((file) => (
                        <li key={file.id} className="py-3 flex items-center justify-between">
                          <div className="flex items-center">
                            <div className="flex-shrink-0">{getFileIcon(file.original_name)}</div>
                            <div className="ml-3">
                              <p className="text-sm font-medium text-gray-900">{file.original_name}</p>
                              <p className="text-xs text-gray-500">{file.formatted_size} • Diunggah {formatDate(file.created_at)}</p>
                            </div>
                          </div>
                          {file.can_download && (
                            <a 
                    ...
                    <p className="text-sm text-gray-500">Belum ada file hasil pekerjaan yang diunggah.</p>
                  )}
                </div>
                
                {/* Attachment Files */}
                <div>
                  <h4 className="text-sm font-medium text-gray-700 mb-3">Lampiran</h4>
                  {files.filter(file => file.file_type === 'attachment').length > 0 ? (
                    <ul className="divide-y divide-gray-200">
                      {files.filter(file => file.file_type === 'attachment').map((file) => (
                        <li key={file.id} className="py-3 flex items-center justify-between">
                          <div className="flex items-center">
                            <div className="flex-shrink-0">{getFileIcon(file.original_name)}</div>
                            <div className="ml-3">
                              <p className="text-sm font-medium text-gray-900">
                                {file.original_name}
                                {file.uploaded_by_me && <span className="ml-2 text-xs text-blue-600">(Unggahan Anda)</span>}
                              </p>
                              <p className="text-xs text-gray-500">
                                {file.formatted_size} • Diunggah oleh {file.uploaded_by_name} • {formatDate(file.created_at)}
                              </p>
                            </div>
                          </div>
                          {file.can_download && (
                    ...
                    <p className="text-sm text-gray-500">Tidak ada file lampiran yang tersedia.</p>
                  )}
                </div>
              </div>
            </div>
            ...
            <div className="bg-white shadow-sm rounded-lg mb-6">
              <div className="px-4 py-5 sm:p-6">
                <h3 className="text-lg font-medium text-gray-900 mb-4">File Hasil Pekerjaan</h3>
                
                {files && files.filter(file => file.file_type === 'deliverable').length > 0 ? (
                  <>
                        ...
                        <li key={file.id} className="py-3 flex justify-between items-center">
                          <div className="flex items-center">
                            ...
                            <div>
                              <p className="text-sm font-medium text-gray-900">{file.original_name}</p>
                              <p className="text-xs text-gray-500">
                                {file.formatted_size || formatFileSize(file.file_size)} • Diunggah pada {formatDate(file.created_at)}
                              </p>
                            </div>
                          </div>
                          <div>
                  ...
                  <div className="text-center py-4">
                    ...
                    <p className="text-sm text-gray-500">
                      Anda belum mengunggah file hasil pekerjaan. Gunakan form di bawah untuk mengunggahnya.
                    </p>
                  </div>
                )}
                
                {/* Form Upload Inline */}
                <div ref={fileUploadSectionRef} className="mt-6 bg-gray-50 p-4 border border-gray-200 rounded-md transition-colors duration-500">
                    <h4 className="font-medium text-gray-800 mb-3">Unggah Hasil Pekerjaan</h4>
                    ...
                    <form onSubmit={handleDeliverySubmit} className="space-y-4">
                      ...
                      <div>
                        ...
                        <div className="flex flex-col space-y-2">
                          <input
                            ...
                            disabled={isSubmitting}
                          />
                          <p className="text-xs text-gray-500">Format yang didukung: PDF, DOC, JPG, PNG, ZIP, dll (max: 50MB)</p>
                        </div>
                        
                        {selectedFiles.length > 0 && (
                          <div className="mt-2">
                            <p className="text-sm text-gray-700">{selectedFiles.length} file dipilih:</p>
                            <ul className="mt-1 text-sm text-gray-500 space-y-1 max-h-24 overflow-y-auto">
                              {selectedFiles.map((file, index) => (
                                <li key={index} className="flex items-center">
                                  ...
                                  <span className="ml-1 text-xs">({formatFileSize(file.size)})</span>
                                </li>
                              ))}
                            </ul>
                          </div>
                      ...
                      {isSubmitting && (
                        <div className="my-2">
                          ...
                          <p className="mt-1 text-xs text-gray-500 text-right">{uploadProgress}% selesai</p>
                        </div>
                      )}
                      ...
                      <div className="flex justify-end">
                        <button
                          ...
                          disabled={isSubmitting || selectedFiles.length === 0 || deliveryMessage.length < 10}
                        >
                          {isSubmitting ? (
                            <>
          ...
          <div className="lg:col-span-1">
            ...
            <div className="bg-white shadow-sm rounded-lg h-full flex flex-col">
              <div className="px-4 py-5 sm:px-6 border-b border-gray-200">
                <h3 className="text-lg font-medium text-gray-900">Komunikasi</h3>
                      ...
                      >
                        ...
                        >
                          ...
                          {message.attachments && message.attachments.length > 0 && (
                            <div className="mt-2">
                              <p className="text-xs font-medium text-gray-500 mb-1">Lampiran:</p>
                              <ul className="space-y-1">
                                {message.attachments.map((attachment, idx) => (
                                  <li key={idx} className="flex items-center">
                                    <div className="flex-shrink-0">{getFileIcon(attachment.original_name)}</div>
                                    <a 
                                      href={attachment.url} 
                                      target="_blank"
                ...
                <form onSubmit={handleMessageSubmit}>
                  ...
                  <div className="mt-3 flex items-center justify-between">
                    <div>
                      ...
                      >
                        <svg className="-ml-1 mr-2 h-5 w-5 text-gray-400" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor">
                    ...
                    <button
                      ...
                      disabled={(!messageText && messageAttachments.length === 0) || isUploading}
                    >
