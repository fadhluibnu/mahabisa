...
const ServiceEdit = ({ user, service: initialService, categories, skills }) => {
  ...
  useEffect(() => {
    console.log('Service initial data:', {
      ...
      requirements_parsed: parseRequirements(
        initialService.requirement_s || []
      ),
    });
  }, [initialService]);
      ...
      try {
        ...
        if (Array.isArray(parsed)) {
          return parsed.map(req =>
            typeof req === 'object' ? req.question || req.text || '' : req
          );
        }
  ...
  const processServicePackages = serviceData => {
    ...
    if (
      ...
      Array.isArray(serviceData.packages) &&
  ...
  const [service, setService] = useState({
    ...
    skills: initialService.skills
      ? initialService.skills.map(skill =>
          typeof skill === 'object' ? skill.id : skill
        )
      : [],
    status: initialService.status || 'active',
    image_url:
      initialService.thumbnail ||
      initialService.image_url ||
      initialService.image ||
      '',
  ...
  const handleSubmit = e => {
    ...
    const filteredRequirements = service.requirements.filter(
      req => req.trim() !== ''
    );
        ...
        if (pkg.features && pkg.features.length > 0) {
          ...
          if (filteredFeatures.length > 0) {
            ...
            filteredFeatures.forEach((feature, featureIndex) => {
              formData.append(
                `packages[${index}][features][${featureIndex}]`,
                feature
              );
            });
          } else {
  ...
  const validateForm = () => {
    ...
    if (service.packages.length === 0) {
      ...
      if (invalidPackages.length > 0) {
        errors.packages =
          'Semua paket harus memiliki nama, harga yang valid, dan waktu pengerjaan';
      }
    }
    ...
    if (
      ...
      !service.requirements.some(req => req.trim() !== '')
    ) {
      errors.requirements = 'Minimal satu persyaratan harus diisi';
    }

    // Validate FAQs
    const invalidFaqs = service.faqs.filter(
    ...
    >
      {isLoading ? (
        ...
        <>
                  ...
                  <div className='grid grid-cols-1 md:grid-cols-3 gap-6'>
                    <div className='md:col-span-2 space-y-6'>
                      <div>
                        ...
                        <p className='mt-1 text-xs text-gray-500'>
                          Judul yang jelas dan menarik akan membantu klien
                          menemukan layanan Anda.
                        </p>
                      </div>
                      ...
                      <div>
                            ...
                            skills.map(skill => (
                              <div key={skill.id} className='flex items-center'>
                                <input
                                  ...
                                  onChange={() => {
                                    const currentSkills = [...service.skills];
                                    const skillIndex = currentSkills.indexOf(
                                      skill.id
                                    );
                        ...
                        <p className='mt-1 text-xs text-gray-500'>
                          Pilih keahlian yang relevan dengan layanan yang Anda
                          tawarkan.
                        </p>
                      </div>
                      ...
                      <div>
                        ...
                        <p className='mt-1 text-xs text-gray-500'>
                          Jelaskan secara detail tentang layanan yang Anda
                          tawarkan, termasuk apa yang akan klien dapatkan.
                        </p>
                      </div>
                    ...
                    <div>
                          ...
                          {imagePreview ? (
                            <div>
                              ...
                              <img
                                ...
                                onError={e => {
                                  console.error(
                                    'Error loading image, trying alternatives'
                                  );
                                  ...
                                  const tryNextAlternative = () => {
                                    if (currentIndex < alternatives.length) {
                                      ...
                                      if (errorElement) {
                                        errorElement.textContent =
                                          'Tidak dapat menampilkan gambar. Silakan upload gambar baru.';
                                      }
                                    }
                                  };
                      ...
                      <div className='mt-2 space-y-1'>
                        <p className='text-xs text-gray-500'>
                          Gambar berkualitas baik akan meningkatkan daya tarik
                          layanan Anda.
                        </p>
                        <p className='text-xs text-gray-400'>
                          Jika gambar tidak muncul, silakan upload gambar baru
                      ...
                      <div className='mt-6'>
                        ...
                        <p className='text-sm text-gray-500 mb-4'>
                          Upload gambar tambahan untuk ditampilkan dalam galeri
                          layanan (opsional, maksimal 5 gambar).
                        </p>
                              ...
                              {service.galleries.map(gallery => (
                                ...
                                >
                                  <div className='aspect-w-1 aspect-h-1 w-full overflow-hidden rounded-md bg-gray-200'>
                                    <img
                                      src={
                                        gallery.image_path.startsWith('http')
                                          ? gallery.image_path
                                    ...
                                    <button
                                      type='button'
                                      onClick={() =>
                                        removeExistingGalleryImage(gallery.id)
                                      }
                        ...
                        {(service.galleries?.length || 0) +
                          ...
                          <p className='text-sm text-yellow-600'>
                            Maksimum 5 gambar galeri diperbolehkan. Hapus
                            beberapa gambar jika Anda ingin mengunggah lebih
                            banyak.
                          </p>
                        )}
                      </div>
                ...
                <div className='p-6' key='requirements-tab'>
                  ...
                  <div className='bg-gray-50 rounded-lg p-4 mb-6'>
                    <p className='text-sm text-gray-700 mb-4'>
                      Beri tahu klien informasi atau bahan apa yang Anda
                      butuhkan untuk memulai pekerjaan:
                    </p>
          ...
          <div className='bg-gray-50 px-4 py-3 text-right sm:px-6 rounded-lg mb-6'>
            <button
              type='button'
              onClick={handleSubmit}
              disabled={isSubmitting}
