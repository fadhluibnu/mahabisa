...
const ProjectDetail = ({ id }) => {
  ...
  const [project, setProject] = useState({
    ...
    progress: 65,
    description: 'Proyek ini melibatkan redesain website e-commerce untuk meningkatkan pengalaman pengguna dan konversi. Meliputi desain ulang halaman produk, checkout, dan implementasi fitur pencarian yang lebih baik.',
    tasks: [
      { id: 1, title: 'Wireframing', status: 'completed', dueDate: '2023-06-03' },
      { id: 2, title: 'Desain UI Homepage', status: 'completed', dueDate: '2023-06-05' },
      { id: 3, title: 'Desain UI Katalog Produk', status: 'completed', dueDate: '2023-06-07' },
      { id: 4, title: 'Desain UI Halaman Checkout', status: 'in-progress', dueDate: '2023-06-10' },
      { id: 5, title: 'Implementasi Revisi', status: 'pending', dueDate: '2023-06-12' },
      { id: 6, title: 'Pengujian & Finalisasi', status: 'pending', dueDate: '2023-06-15' }
    ],
    milestones: [
      { id: 1, title: 'Persetujuan Wireframe', status: 'completed', date: '2023-06-03', amount: 'Rp900.000' },
      { id: 2, title: 'Desain UI Selesai', status: 'completed', date: '2023-06-07', amount: 'Rp1.350.000' },
      { id: 3, title: 'Implementasi Revisi', status: 'pending', date: '2023-06-12', amount: 'Rp1.350.000' },
      { id: 4, title: 'Proyek Selesai', status: 'pending', date: '2023-06-15', amount: 'Rp900.000' }
    ],
    messages: [
      { id: 1, sender: 'Client', content: 'Bagaimana progress redesain website?', date: '2023-06-05T09:30:00' },
      { id: 2, sender: 'You', content: 'Saya sudah menyelesaikan desain UI untuk homepage dan sedang mengerjakan halaman katalog produk.', date: '2023-06-05T10:15:00' },
      { id: 3, sender: 'Client', content: 'Terlihat bagus. Saya punya beberapa saran untuk halaman checkout.', date: '2023-06-06T14:22:00' }
    ],
    files: [
      { id: 1, name: 'wireframe_v1.pdf', size: '2.4 MB', date: '2023-06-02', type: 'document' },
      { id: 2, name: 'homepage_design.fig', size: '5.1 MB', date: '2023-06-04', type: 'design' },
      { id: 3, name: 'catalog_pages.fig', size: '4.8 MB', date: '2023-06-06', type: 'design' },
      { id: 4, name: 'feedback_client.docx', size: '1.2 MB', date: '2023-06-06', type: 'document' }
    ],
    clientInfo: {
  ...
  return (
    <FreelancerLayout title="Detail Proyek" subtitle="Kelola proyek dan pantau progres">
      {/* Project Header */}
            ...
            <div className="flex items-center space-x-3">
              <span className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium ${
                ...
                {project.status === 'completed' ? 'Selesai' :
                 project.status === 'in-progress' ? 'Sedang Berjalan' :
                 project.status === 'pending' ? 'Tertunda' : project.status}
              </span>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 mb-6">
        <div className="bg-white rounded-xl shadow-sm p-6">
          ...
          <p className="text-lg font-bold text-gray-800">{formatDate(project.deadline)}</p>
        ...
        <div className="bg-white rounded-xl shadow-sm p-6">
          <h3 className="text-sm font-medium text-gray-500 mb-1">Total Anggaran</h3>
        ...
        <div className="bg-white rounded-xl shadow-sm p-6">
          <h3 className="text-sm font-medium text-gray-500 mb-1">Sisa Pembayaran</h3>
        ...
        <div className="p-6">
          ...
          {activeTab === 'overview' && (
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <h4 className="text-md font-medium text-gray-900 mb-3">Tugas Terbaru</h4>
                    ...
                    {project.tasks.slice(0, 3).map((task) => (
                      <div key={task.id} className="mb-3 last:mb-0">
                        <div className="flex items-center">
                          ...
                          <span className="ml-auto text-xs text-gray-500">{formatDate(task.dueDate)}</span>
                        </div>
                      </div>
                    ))}
                    <button 
                ...
                <div>
                  <h4 className="text-md font-medium text-gray-900 mb-3">Milestone Terbaru</h4>
                  <div className="bg-gray-50 rounded-lg p-4">
                    {project.milestones.slice(0, 3).map((milestone) => (
                      <div key={milestone.id} className="mb-3 last:mb-0">
                        <div className="flex items-center">
                          ...
                          <span className="ml-auto text-xs text-gray-500">{formatDate(milestone.date)}</span>
                        </div>
                      </div>
                    ))}
                    <button 
          ...
          {activeTab === 'tasks' && (
            <div>
              ...
              {/* Add Task Form */}
              <div id="add-task-form" className="bg-gray-50 p-4 rounded-lg mb-6 hidden">
                <div className="grid grid-cols-1 md:grid-cols-6 gap-4">
                  ...
                  <div className="md:col-span-2">
                    <label htmlFor="task-duedate" className="block text-sm font-medium text-gray-700 mb-1">Tenggat Waktu</label>
                    <input
                      type="date"
                      id="task-duedate"
                      className="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-md"
                          ...
                          <div className="mt-2 flex items-center text-xs text-gray-500">
                            ...
                            <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                              ...
                              {task.status === 'completed' ? 'Selesai' :
                               task.status === 'in-progress' ? 'Dalam Proses' : 'Tertunda'}
                            </span>
                          </div>
                        </div>
          ...
          {activeTab === 'milestones' && (
            <div>
              <div className="flex justify-between items-center mb-4">
                <h3 className="text-lg font-medium text-gray-900">Milestone Proyek</h3>
                <button 
                  type="button"
                  className="inline-flex items-center justify-center px-4 py-2 border border-transparent text-sm font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500"
                >
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          <div className="mt-2 flex items-center text-xs text-gray-500">
                            ...
                            <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                              ...
                              {milestone.status === 'completed' ? 'Selesai' : 'Tertunda'}
                            </span>
                          </div>
                        </div>
          ...
          {activeTab === 'files' && (
            <div>
              <div className="flex justify-between items-center mb-4">
                <h3 className="text-lg font-medium text-gray-900">File Proyek</h3>
                <button 
                  type="button"
                  className="inline-flex items-center justify-center px-4 py-2 border border-transparent text-sm font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500"
                >
                    ...
                    <li key={file.id} className="px-6 py-4">
                      <div className="flex items-center">
                        <div className="flex-shrink-0">
                          <svg 
                            className={`h-10 w-10 ${
                              file.type === 'design' ? 'text-purple-500' : 
                              file.type === 'document' ? 'text-blue-500' : 'text-gray-500'
                            }`} 
                        ...
                        <div className="ml-4">
                          <p className="text-sm font-medium text-indigo-600 hover:text-indigo-500 cursor-pointer">{file.name}</p>
          ...
          {activeTab === 'client' && (
            <div>
              <h3 className="text-lg font-medium text-gray-900 mb-4">Informasi Klien</h3>
              ...
              <div className="bg-white rounded-lg shadow p-6">
                <div className="flex items-center mb-6">
                  ...
                  />
                  <div>
                    <h4 className="text-lg font-medium text-gray-900">{project.clientInfo.name}</h4>
                    <p className="text-sm text-gray-500">Contact Person: {project.clientInfo.contactPerson}</p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    <div className="flex items-start">
                      <svg className="mt-0.5 h-5 w-5 text-gray-400 mr-2" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
                      </svg>
                      <div>
                        <p className="text-sm font-medium text-gray-900">Email</p>
                        <p className="text-sm text-gray-500">{project.clientInfo.email}</p>
                      </div>
                    </div>
                    <div className="flex items-start">
                      <svg className="mt-0.5 h-5 w-5 text-gray-400 mr-2" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-gray-500">{project.clientInfo.phone}</p>
                      </div>
                    </div>
                  </div>
                  
                  <div>
                    <p className="text-sm font-medium text-gray-900 mb-2">Proyek Sebelumnya</p>
                    <p className="text-sm text-gray-500 italic">Belum ada proyek sebelumnya dengan klien ini.</p>
                  </div>
                </div>
