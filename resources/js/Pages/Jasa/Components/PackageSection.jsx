...
const PackageSection = ({ 
  ...
  const handleOrderClick = () => {
    if (!isAuthenticated) {
      ...
      alert('Anda harus menjadi klien untuk memesan jasa ini. Silakan ubah peran Anda ke klien di pengaturan akun.');
    }
  };
  ...
  if (packages.length === 0) {
    return (
      <div className="sticky top-24 bg-white rounded-2xl shadow-md overflow-hidden p-6">
        <p className="text-center text-gray-500 mb-4">Tidak ada paket tersedia untuk jasa ini.</p>
        ...
        >
          ...
          <div className="mb-6">
            <div className="flex items-center justify-between mb-3">
              ...
              <span className="text-sm font-medium text-gray-900">{pkg.delivery_time} hari</span>
            </div>
            <div className="flex items-center justify-between mb-3">
            ...
            <ul className="space-y-2">
              {pkg.features && (Array.isArray(pkg.features) 
                ...
                <li key={index} className="flex items-start">
                  ...
                  <span className="text-sm text-gray-700">{typeof feature === 'string' ? feature.trim() : feature}</span>
                </li>
              ))}
            </ul>
          </div>
          ...
          <div className="space-y-3">
            ...
            <Link
              href={isAuthenticated ? `/messages/create?recipient=${serviceId}` : `/auth?form=login&redirect=/jasa/${serviceId}`}
